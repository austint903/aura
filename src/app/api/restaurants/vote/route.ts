...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (existingVote) {
            return NextResponse.json({ error: "you have already voted for this restaurant" }, { status: 400 });
        }
        ...
        //call teh function on restaurants table to increment
        ...
        const updatedTotal = data[0]?.total_points; //value returned from the rpc is an array, take the first element, if data[0] exists then access total points, if not return undefiend
        ...
        //return the value total_points to frontend
        return NextResponse.json({ total_points: updatedTotal }, { status: 200 });
